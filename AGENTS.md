...
- **v1.11.0** adiciona 7 novas seções: `#glossario` (20 siglas com busca), `#brasil-vs-mundo` (12 países comparados), `#calculadora-pessoal` (input de renda + 27 UFs + 8 categorias → imposto por esfera + equivalências), `#linha-fiscal-1985` (10 mandatos com dívida/PIB), `#quiz-imposto` (10 perguntas com pontuação e feedback), `#cards-sociais` (6 templates PNG 1080x1080 via Canvas API). Mais PWA: `manifest.json` + `sw.js` (service worker cache-first) + meta tags. Funções JS: `brlFmt`, `calcIR`, `initCalcPessoal`, `initQuiz`, `renderQuiz`, `responderQuiz`, `mostrarResultado`, `nextQuiz`, `restartQuiz`, `initCardsSociais`, `downloadCard`. CSV `brasil_vs_mundo` com 12 linhas × 8 métricas.
- **v1.12.0** revisão tipográfica completa: `.parlTable` corrigido (valores não quebram mais no meio), `.grid two` com `minmax(280px, 1fr)`, line-heights ajustados em todos os componentes densos (miniCard, glossItem, lfItem, impostoItem, persona, calcPessoal, quiz). font-sizes +1px onde apropriado, paddings +2px, gaps +2px. Tipografia profissional sem perder densidade.
